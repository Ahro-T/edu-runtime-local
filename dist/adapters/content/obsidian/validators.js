...
export function validateVault(nodes, templates) {
    const errors = [];
    // Unique node ids
    ...
    for (const node of nodes) {
        // Valid pillar
        if (!VALID_PILLARS.includes(node.pillar)) {
        ...
        // Valid mastery stage
        if (!VALID_MASTERY_STAGES.includes(node.masteryStageTarget)) {
            errors.push(`Node "${node.id}": invalid mastery_stage_target "${node.masteryStageTarget}"`);
        }
        // Valid teacher prompt mode
        if (!VALID_TEACHER_MODES.includes(node.teacherPromptMode)) {
