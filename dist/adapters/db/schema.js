import { pgTable, text, integer, real, timestamp, jsonb, unique, check, } from 'drizzle-orm/pg-core';
...
export const reviewJobs = pgTable('review_jobs', {
    ...
    check('review_jobs_job_type_check', sql `${table.jobType} IN ('review', 'retry', 'reminder')`),
    check('review_jobs_status_check', sql `${table.status} IN ('pending', 'running', 'done', 'failed', 'cancelled')`),
]);
