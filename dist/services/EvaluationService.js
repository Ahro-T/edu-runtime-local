...
export class EvaluationService {
    ...
    engine;
    logger;
    constructor({ learnerStateStore, learnerEventStore, submissionStore, contentRepository, evaluationEngine, logger }) {
    ...
    async evaluateSubmission(submissionId) {
        ...
        if (!submission)
            throw new AppError('SESSION_NOT_FOUND', `Submission not found: ${submissionId}`);
        const node = await this.content.getNodeById(submission.nodeId);
        if (!node)
            throw new AppError('NODE_NOT_FOUND', `Node not found: ${submission.nodeId}`);
        const template = await this.content.getTemplateByNodeId(submission.nodeId);
        if (!template)
            throw new AppError('TEMPLATE_NOT_FOUND', `No template for node: ${submission.nodeId}`);
        // Check degraded mode
