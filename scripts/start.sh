...
if [ "$MODE" = "democlaw" ]; then
  echo "[start] Mode: democlaw (attaching edu-runtime to existing democlaw stack)..."

  # Check that democlaw-net exists
  if ! $CONTAINER network inspect democlaw-net > /dev/null 2>&1; then
    echo "[start] ERROR: democlaw-net not found. Start democlaw first: cd ../democlaw && ./scripts/start.sh"
    exit 1
  fi

  echo "[start] Building and starting edu-runtime services..."
  $COMPOSE -f docker-compose.democlaw.yml up -d --build
  ...
  echo ""
  echo "========================================="
  echo "  Edu Runtime is running (democlaw mode)"
  echo "========================================="
