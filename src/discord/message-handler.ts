...
import type { EvaluationResult } from './formatter.js';

const BASE_URL = `http://localhost:${process.env['PORT'] ?? 3000}`;

export function registerMessageHandler(client: Client, logger: Logger): void {
  ...
  client.on('messageCreate', async (message: Message) => {
    ...
    if (!channelManager.isStudyChannel(channelName)) return;
    ...
    try {
      ...
      if (!submissionRes.ok) {
        const err = await submissionRes.json() as { message?: string; code?: string };
      ...
      const { submission } = await submissionRes.json() as { submission: { id: string } };
      ...
      if (!evalRes.ok) {
        const err = await evalRes.json() as { message?: string; code?: string };
      ...
      const { evaluation } = await evalRes.json() as { evaluation: EvaluationResult & { submissionId: string } };
      ...
      const nodeTitle = nodeRes.ok
        ? ((await nodeRes.json() as { node: { title: string } }).node.title)
